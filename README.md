...
### 💡 操作情境示例：團隊 A / B 共用同一個 Bot，如何用多頻道區隔專案？

假設你有一個公司團隊，想讓不同專案共用同一個機器人，但檔案要分開管理：

- `@team_a_media`：專門給 A 專案成員上傳素材  
- `@team_b_media`：專門給 B 專案成員上傳素材  
- `-1001234567890`：一個私有群組，用來備份最終成品

你可以這樣設定 `.env`：

```env
BOT_TOKEN=1234567890:AA...your_bot_token...
CHANNEL_NAME=@team_a_media,@team_b_media,-1001234567890
```

這樣的效果是：

- Bot 會同時監聽這三個頻道 / 群組上傳的檔案，全部都會出現在同一套 Web 管理介面裡。
- Web 後台「檔案管理 / 圖床模式」中，每一個檔案都會顯示「來源頻道/群組」，你可以用搜尋列輸入 `@team_a_media` 或 `@team_b_media` 快速過濾。
- 在 Web 上傳時：
  - 上傳區底下會出現一個「選擇頻道 / 群組」的輸入框＋下拉清單。
  - 你可以輸入 `team_a` 或 `team_b` 來篩選，點選後，當次上傳就會被送到對應的頻道。
  - 如果輸入框留空，系統會把檔案傳到第一個頻道（上述例子中的 `@team_a_media`），作為「預設頻道」。

這樣一來：

- 同一個 Bot + 同一套 Web 後台，就可以同時服務多個專案 / 多個團隊。
- 檔案來源一目瞭然，又可以依頻道/群組快速搜尋和管理。
